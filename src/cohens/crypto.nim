import std/[math, sequtils, sugar, random, enumerate]
...
proc generatePublicKey*(p: int64, k: int64, b: int64 = 1000): seq[int64] =
  var r: Rand = initRand()
  let uValues: seq[int64] = ((0.int64)..k).toSeq().map(x => r.rand(-b..b).int64)
  let vValues: seq[int64] = ((0.int64)..k).toSeq().map(x => r.rand(0..int(ceil(p/(2*k)))).int64)

  return zip(uValues, vValues).mapIt(it[0] * p + it[1])

proc generatePublicPrivateKeyPair* (k: int64, b: int64 = 1000): (int64, seq[int64]) =
  let privateKey = generatePrivateKey()
  let publicKey = generatePublicKey(privateKey, k, b)
  return (privateKey, publicKey)

proc encrypt*(message: seq[byte], publicKey: seq[int64]): seq[int64] =
